...
<style>
  ...
  /* Binary "HUB exists" cue (this fork). The `has-note` class is added at
     runtime by the hubExists source, so it is invisible to Svelte's scoped-CSS
     pruner — hence :global. */
  :global(.day.has-note:not(.active)) {
    font-weight: 700;
