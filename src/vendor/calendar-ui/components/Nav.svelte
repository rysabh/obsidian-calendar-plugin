<script lang="ts">
  ...
  // Header click targets (this fork): clicking the month/year word
  // creates-or-opens the Month/Year HUB for the *displayed* period.
...
<div class="nav" class:is-mobile="{isMobile}">
  <h3 class="title">
    <span
      class="month"
      class:clickable="{onClickMonth}"
      on:click="{onClickMonth && (() => onClickMonth(displayedMonth))}"
      >{displayedMonth.format("MMM")}</span
    >
    <span
      class="year"
      class:clickable="{onClickYear}"
      on:click="{onClickYear && (() => onClickYear(displayedMonth))}"
